import type { ReactNode } from 'react';
import { cn } from '@/lib/utils';
import type { Card, Suit } from '@/lib/blackjack';
import { Spade, Heart, Club, Diamond } from 'lucide-react';

const suitIcons: Record<Suit, ReactNode> = {
  '♠': <Spade className="h-full w-full" />,
  '♥': <Heart className="h-full w-full fill-current" />,
  '♦': <Diamond className="h-full w-full fill-current" />,
  '♣': <Club className="h-full w-full fill-current" />,
};

export function CardComponent({ card, hidden, className }: { card?: Card; hidden?: boolean; className?: string }) {
  if (hidden) {
    return (
      <div className={cn("relative h-36 w-24 rounded-lg bg-secondary border-2 border-dashed border-muted-foreground/50 shadow-lg", className)}>
        <div className="flex h-full w-full items-center justify-center">
            <div className="h-16 w-12 rounded-md bg-primary/20"></div>
        </div>
      </div>
    );
  }

  if (!card) return null;

  const isRed = card.suit === '♥' || card.suit === '♦';

  return (
    <div className={cn("relative h-36 w-24 rounded-lg bg-card shadow-lg", className)}>
      <div className={cn(
        "flex h-full w-full flex-col justify-between p-2 font-black",
        isRed ? 'text-destructive' : 'text-foreground'
      )}>
        <div className="flex flex-col items-start">
          <span className="text-2xl leading-none">{card.rank}</span>
          <span className="h-4 w-4">{suitIcons[card.suit]}</span>
        </div>
        <div className="flex justify-center items-center">
             <span className="h-8 w-8">{suitIcons[card.suit]}</span>
        </div>
        <div className="flex flex-col items-end rotate-180">
          <span className="text-2xl leading-none">{card.rank}</span>
          <span className="h-4 w-4">{suitIcons[card.suit]}</span>
        </div>
      </div>
    </div>
  );
}
